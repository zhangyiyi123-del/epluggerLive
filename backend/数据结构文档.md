...
- 用户与认证：`user`（含任职状态、人员库同步时间；`id` 与 epWork 人员库主键对齐的同步路径由业务写入）
- epWork 集成：`epwork_sso_nonce`、`epwork_sso_exchange_code`（外链 SSO 落地）、`epwork_personnel_sync_cursor`（人员增量同步游标）
...
erDiagram
  ...
  user ||--o{ epwork_sso_exchange_code : sso_exchange
```
...
|---|---|---|---|
| 用户与认证 | 用户 | `user` | `id` |
| epWork 集成 | SSO nonce 消费记录 | `epwork_sso_nonce` | `nonce` |
| epWork 集成 | SSO 交换码 | `epwork_sso_exchange_code` | `code` |
| epWork 集成 | 人员同步游标 | `epwork_personnel_sync_cursor` | `cursor_key` |
...
| `sso_id` | VARCHAR(255) | NULL | SSO 标识（对应人员库 `USER_ID`） |
| `employment_status` | VARCHAR(16) | NOT NULL, DEFAULT `'ACTIVE'` | 任职状态：`ACTIVE`（在职）/`INACTIVE`（非在职），与 epWork 人员库同步 |
| `last_synced_at` | TIMESTAMP | NULL | 最近一次从 epWork 人员库同步的时间 |
| `created_at` | DATETIME(6) | NOT NULL | 创建时间 |

> `id`：列定义为 `AUTO_INCREMENT`；epWork 人员同步等路径由应用显式写入主键，以与人员库 `BIZ_PERSON.ID` 对齐。

索引：
- `UQ(phone)`
- `IDX idx_user_sso_id (sso_id(100))`
- `IDX idx_user_employment_status (employment_status)`
- `IDX idx_user_last_synced_at (last_synced_at)`
...
| `consumed_at` | DATETIME(6) | NOT NULL, DEFAULT CURRENT_TIMESTAMP(6) | 消费时间 |
...
| `used` | TINYINT(1) | NOT NULL, DEFAULT 0 | 是否已兑换 |

索引：`IDX idx_sso_exchange_expires (expires_at)`

### 5.1c epWork 人员同步 — `epwork_personnel_sync_cursor`

| 字段 | 类型 | 约束 | 说明 |
|---|---|---|---|
| `cursor_key` | VARCHAR(64) | PK | 游标业务键（如同步任务维度） |
| `last_update_time` | TIMESTAMP | NULL | 上游人员数据上次已处理到的更新时间 |
| `updated_at` | TIMESTAMP | NOT NULL, ON UPDATE CURRENT_TIMESTAMP | 本行最近写入时间 |

> 无外键；用于增量拉取 epWork 人员库时的断点记录。
...
- **用户与 epWork**：同步写入的用户 `id` 与人员库主键对齐；`sso_id` 对齐人员库用户标识。列表/排行榜等场景常按 `employment_status = 'ACTIVE'` 过滤在职用户。
- **人员同步游标**：`epwork_personnel_sync_cursor` 仅存游标状态，与 `user` 无外键，避免同步任务与业务用户生命周期耦合。
...
- `V26` 种子演示用户 `13800138010` 及对应 `user_points`
- `V27` 为已存在的该手机号用户补齐与种子用户一致的密码哈希
- `V28` 全量修正用户 `password_hash` 为可校验的 123456 BCrypt（`$2b$10$…`）
- `V29` 新增 epWork SSO 表 `epwork_sso_nonce`、`epwork_sso_exchange_code`
- `V30` `user` 新增 `employment_status`、`last_synced_at` 及索引
- `V31` 新增人员同步游标表 `epwork_personnel_sync_cursor`
